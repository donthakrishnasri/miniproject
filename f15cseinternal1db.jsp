...
<%
    ...
    try{
          ...
          Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mist","root","root");
              ...
              System.out.println(""+srno.length);
              String sql = "insert into f15cseinternal1_db values('"+srno[i]+"','"+rollno[i]+"','"+sub1[i]+"','"+sub2[i]+"','"+sub3[i]+"','"+sub4[i]+"','"+sub5[i]+"','"+sub6[i]+"','"+lab1[i]+"','"+lab2[i]+"')";
       ...
       //  st.setString(9,request.getParameter("lab1[i]"));
         //st.setString(10,request.getParameter("lab2[i]"));
          ...
          if(k>0)
          System.out.println("internal1 are inserted successfully======"+k);
          else{
               System.out.println("internal1 records  failed======"+k);
          }
